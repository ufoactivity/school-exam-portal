...
st.info("💡 終極升級：已實裝「第一節(※)指定班級」功能，且第一節(※)與第二節(△)將完美鎖定於同一班級監考！解決 index out of bounds 錯誤，完美區分靜態數據與動態報表。")
...
with col2:
    ...
    if file_assign:
        ...
        class_list = [clean_str(c) for c in class_names_raw]

    st.write("---")
    # 【修改重點】：新增第一節(※)綁定班級的 UI 介面
    ...
    if st.button("🗑️ 清除所有設定", use_container_width=True):
        ...
        st.session_state.bind_rules = pd.DataFrame([{"老師": None, "班級": None}] * 3) # 同步清空綁定記憶
        st.rerun()
...
if st.button("🚀 啟動終極全自動排班系統", type="primary", use_container_width=True):
    if not all([file_quota, file_list, file_type, file_assign]):
        ...
        try:
            ...
            for day_start in [0, 5]:
                ...
                for p in avail_j1:
                    if p in t2c_map and schedule_dict[p][j1] == "※":
                        ...
                        if assigned_matrix[target_idx, j1] is None:
                            assigned_matrix[target_idx, j1] = p
                        else:
                            rem_j1.append(p) # 該班已經有人了，退回一般池
                    else:
                        rem_j1.append(p)
                
                # 剩下未綁定的老師，隨機填補剩下的空缺班級
                r_ptr = 0
            ...
            st.success("🎉 排班完成！「第一節※綁定」及「連堂同班鎖定」邏輯已完美發揮作用！")
