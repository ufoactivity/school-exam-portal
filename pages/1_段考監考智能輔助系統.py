...
st.info("💡 系統已升級為「兩階段作業模式」。請利用下方頁籤切換【階段一：試卷催繳】與【階段二：監考排定】。兩階段獨立運作，互不干擾！")

# --- 狀態記憶體初始化 (Session State) ---
# 確保網頁重整、按鈕點擊或切換頁籤時，產生的檔案不會閃退消失
...
# ==========================================
# 📌 重點修改：使用 st.tabs 將原本獨立的兩支程式優雅地整合在同一頁面上
tab1, tab2 = st.tabs(["📑 階段一：試卷催繳通知單", "📅 階段二：段考監考全自動排班"])
...
with tab1:
    ...
    st.markdown("自動讀取上傳 Excel 的工作表名稱作為「考試類型」，並彙整每位老師未繳交的試卷清單，產生獨立分頁的 Word 通知單。")
    ...
    with col1_p1:
        st.markdown("##### ⚙️ 參數設定")
        deadline = st.text_input("📅 繳交截止日", value="6/26", help="例如：6/26 或 112年6月26日", key="p1_deadline")
    ...
    if st.button("🚀 一鍵產出催繳通知單", use_container_width=True, type="primary", key="btn_p1"):
        ...
        elif uploaded_file_p1 is None:
            st.warning("⚠️ 老師，請先上傳名單檔案喔！")
        else:
            try:
                excel_file = pd.ExcelFile(uploaded_file_p1)
                sheet_name = excel_file.sheet_names[0]
                df = pd.read_excel(uploaded_file_p1, sheet_name=sheet_name)
    ...
    if st.session_state['processed_p1'] and st.session_state['docx_data_p1']:
        st.success("✅ 完美達成！所有催繳通知單已為您排版完畢，每一位老師獨立一頁。")
        c1, c2, c3 = st.columns([1, 2, 1])
        with c2:
            st.download_button(
                label="📥 點我下載：試卷催繳通知單 (Word檔)",
                data=st.session_state['docx_data_p1'],
                file_name=f"試卷催繳通知單_{deadline.replace('/', '')}.docx",
...
# ---------------------------------------------------------
# 【階段二：段考監考全自動排班】 (原封不動保留您的終極完全體)
# ---------------------------------------------------------
with tab2:
    st.subheader("📅 階段二：段考監考全自動化系統 (終極完全體)")
    ...
    with col2_p2:
        ...
        if file_quota:
            ...
            selected_sheet = st.selectbox("👇 選擇考試項目：", xls.sheet_names, key="p2_sheet")
        ...
        if file_list:
            ...
            flex_names = st.multiselect("🛡️ 優先時數不大於名單：", options=teacher_list, key="p2_flex")

        st.write("")
        c_d1, c_d2 = st.columns(2)
        with c_d1: d1_date = st.date_input("📅 預備日期1：", datetime.now(), key="p2_d1")
        with c_d2: d2_date = st.date_input("📅 預備日期2：", datetime.now(), key="p2_d2")
        
        force_run = st.checkbox("⚠️ 忽略健檢警告，強制執行", key="p2_force")
        
        # 清除按鈕只清除階段二的檔案與設定，保證安全防呆
        if st.button("🗑️ 清除階段二設定", use_container_width=True):
            st.session_state['results_p2'] = None
    ...
    if st.session_state['results_p2']:
        ...
        with c1: 
            st.download_button("📥 1. 監考總表", res['orig'], "監考總表.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="dl_p2_1")
        with c2: 
            st.download_button("📥 2. 監考一覽表(保留格式)", res['assign'], "監考一覽表_分配完成.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, type="primary", key="dl_p2_2")
        with c3: 
            if res.get('pub'): 
                st.download_button("📥 3. 公布版套印總表", res['pub'], "公布版總表.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="dl_p2_3")
        with c4:
            if res.get('label'): 
                st.download_button("📥 4. 標籤列印(完整版)", res['label'], "標籤列印_完整版.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, type="primary", key="dl_p2_4")
