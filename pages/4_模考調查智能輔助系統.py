...
st.info("💡 修正說明：已全面升級「智慧讀取引擎」，完美修復真實 .xlsx (PK檔頭) 誤判問題，徹底相容校務系統各類匯出檔！")
...
# ==========================================
# 2. 輔助功能定義 (包含升級版智慧讀取引擎)
# ==========================================
def smart_read_excel(file_uploader, header=0):
    """
    💡 AI 工程師優化：智慧讀取引擎 4.0
    專門解決 [PK...] 錯誤，強制使用 openpyxl 處理真實 .xlsx 檔案
    """
    ...
    # 1. 攔截真實 .xlsx (ZIP 壓縮檔特徵 'PK')，強制掛載 openpyxl 引擎
    if raw_bytes.startswith(b'PK'):
        try:
            return pd.read_excel(io.BytesIO(raw_bytes), engine='openpyxl', header=header).fillna("")
        except Exception:
            pass
            
    # 2. 嘗試標準 Pandas 讀取
    try:
        return pd.read_excel(io.BytesIO(raw_bytes), header=header).fillna("")
    except Exception:
        pass
    ...
    for enc in ['utf-8', 'big5', 'cp950']:
        try:
            ...
            if '<table' in html_str.lower():
                dfs = pd.read_html(io.StringIO(html_str), header=header)
                if dfs:
                    return max(dfs, key=lambda x: x.shape[0]).fillna("")
        except Exception:
            pass
    ...
    for sep in [',', '\t']:
        for enc in ['utf-8', 'big5', 'cp950', 'utf-8-sig']:
            try:
                ...
                if df.shape[1] > 1: return df.fillna("")
            except Exception:
                pass
                
    head_str = raw_bytes[:30].decode('utf-8', errors='ignore').replace('\n', ' ')
...
if st.button("🚀 開始智慧排考運算", type="primary", use_container_width=True):
    if not all([file_target, file_short, file_exam, file_teacher]):
        ...
        with st.spinner("系統正在執行智慧運算中..."):
            try:
                # 💡 AI 工程師優化：全面套用 smart_read_excel 智慧讀取引擎
                df_short_map = smart_read_excel(file_short)
                df_exam_map = smart_read_excel(file_exam)
                df_target = smart_read_excel(file_target)
                df_teacher = smart_read_excel(file_teacher)
