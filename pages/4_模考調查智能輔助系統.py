...
st.title("📊 試務組-模考調查智能輔助系統 (A4極致排版與條列費用版)")
st.info("💡 試務組終極排版：底部費用已改為清晰的「條列式呈現」。系統已啟動 A4 極限微調排版，確保大班級也能完美印在同一頁不被截斷！115.05.21增修")
...
with col1:
    st.subheader("📂 1. 上傳調查表單")
    file_survey = st.file_uploader(
        "📥 上傳學生意願調查表 (系統將自動尋找並套用表內費用)", 
        type=['xlsx', 'csv'], 
        key=f"mock_f1_{st.session_state.mock_uploader_key}"
    )
    ...
    st.markdown("📝 **列印優化說明**：")
    st.success("已縮小邊界與列高，搭配 A4 紙張自動縮放，確保 40 人以上的大班級依然能完美塞進一頁 A4 之中！")

with col2:
    st.subheader("⚙️ 2. 收費檢核與測驗設定")
    mock_name = st.text_input("🎯 模擬考名稱", value="114學年度高職統測模擬考(全學年共5次)")
    base_fee = st.number_input("💰 預設單次報名費", min_value=0, max_value=5000, value=160, step=10)
    
    special_fee_dict = {}
    if file_survey and detected_categories:
        st.markdown("### 💰 各類群單次費用 (已自動從檔案擷取)")
        st.caption("💡 系統已自動將您上傳表單中的金額帶入，請檢核。若有遺漏或需微調，可直接點擊表格修改：")
        ...
        st.caption("💡 老師，請先於左側上傳調查表單，系統會自動擷取表內的費用供您檢核喔！")

    st.write("")
    if st.button("🗑️ 清除所有設定 / 重新上傳", use_container_width=True):
        st.session_state.mock_processed = False
        st.session_state.mock_excel_data = None
...
if st.button("🚀 啟動智能分流與結算", type="primary", use_container_width=True):
    if not file_survey:
        st.error("🚨 老師，請記得上傳【學生意願調查表】喔！")
    else:
        with st.spinner("系統正在高速進行 5 倍收費放大與【A4自適應抗截斷排版】統計中..."):
            try:
                file_survey.seek(0)
                if file_survey.name.endswith('.csv'):
                ...
                df_all['座號'] = get_str_col(df_raw, ['座號', '學號'])
                ...
                df_details_raw = df_clean[['班級', '座號', '姓名', '報考類群', '單次應繳費用', '五次總繳費金額']].copy()
                ...
                with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
                    ...
                    # 設定黃金比例欄寬
                    ws_details.set_column('A:A', 10) # 班級
                    ws_details.set_column('B:B', 9)  # 座號
                    ws_details.set_column('C:C', 10) # 姓名
                    ws_details.set_column('D:D', 24) # 報考類群
                    ws_details.set_column('E:E', 13) # 單次費用
                    ws_details.set_column('F:F', 15) # 總金額
                    ws_details.set_column('G:G', 16) # 學生簽名
                    ...
                    data_format = workbook.add_format({'border': 1, 'align': 'center', 'valign': 'vcenter', 'font_size': 11}) # 字體縮為 11 節省空間
                    ...
                    headers = ['班級', '座號', '姓名', '報考類群', '單次費用(參考)', '應繳總金額(5次)', '學生簽名']
                    ...
                    for cls_name in unique_classes:
                        ...
                        ws_details.set_row(current_row, 24) # 列高微調
                        current_row += 1
                        ...
                        for _, row in df_cls.iterrows():
                            ...
                            ws_details.write(current_row, 2, str(row['姓名']), data_format)
                            ws_details.write(current_row, 3, str(row['報考類群']), data_format)
                            ws_details.write(current_row, 4, row['單次費用(參考)'], data_format)
                            ws_details.write(current_row, 5, row['應繳總金額(5次)'], data_format)
                            ws_details.write(current_row, 6, '', data_format) 
                            ws_details.set_row(current_row, 16) # 列高壓縮，防截斷
                            current_row += 1
                        ...
                        ws_details.write(current_row, 1, f'共 {cls_count} 人報名', total_format)
                        ...
                        ws_details.write(current_row, 5, cls_total_amt, total_format)
                        ws_details.write(current_row, 6, '導師簽章：', total_format) 
                        ...
                        page_breaks.append(current_row) # 確保分頁線切在此班最後一行的正下方
                    ...
                    ws_details.write(current_row, 5, df_details_raw['應繳總金額(5次)'].sum(), grand_format)
                    ws_details.write(current_row, 6, '', grand_format)
...
if st.session_state.mock_processed:
    st.balloons()
    st.success("🎉 排版防截斷處理完成！底部費用已轉為條列式，大班級也能安心印在同一張 A4 囉！")
    
    st.download_button(
        label="📥 點擊下載【模擬考收費與各班未報考人數交叉檢核總表】",
        data=st.session_state.mock_excel_data,
        file_name=f"{mock_name}_條列費用抗截斷版_{datetime.now().strftime('%Y%m%d')}.xlsx",
