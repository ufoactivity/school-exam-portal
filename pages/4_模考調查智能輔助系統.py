...
st.title("📊 教務處-模擬考調查智能輔助系統 (普高/技高雙軌全流程版)")
st.info("💡 試務組終極進化：系統已支援「技高統測」與「普高學測」雙軌調查表產出！第二階段結算將自動對應不同學制的收費與表單。")
...
with tab1:
    ...
    st.markdown("上傳各年級的純淨學生名單（只需包含班級、座號、姓名），系統將為您自動加上收費欄位與右側代碼對照表，產出發給導師的公版格式。")
    
    col1_t1, col2_t1 = st.columns([1, 1], gap="large")
    
    with col1_t1:
        file_roster = st.file_uploader("📥 上傳學生原始名條 (包含班級、座號、姓名)", type=['xlsx', 'csv'], key="roster_uploader")
        ...
        if not file_roster:
            ...
            with st.spinner("正在合成公版調查表..."):
                try:
                    ...
                    df_temp['座號'] = get_str_col(df_roster, ['座號', '學號'])
                    ...
                    with pd.ExcelWriter(output_template, engine='xlsxwriter') as writer:
                        ...
                        worksheet.merge_range(0, 0, 0, 5, template_name, title_format)
                        worksheet.set_row(0, 25)
                        
                        headers = ['班級', '座號', '姓名', '單次費用', '報考類組', '簽名']
                        ...
                        worksheet.write(1, 7, "代碼", mapping_head_format)
                        worksheet.write(1, 8, "類別", mapping_head_format)
                        
                        map_row = 2
                        target_mapping = STANDARD_MAPPING_VOC if "技高" in school_type else STANDARD_MAPPING_GEN
                        for code, name in target_mapping.items():
                            worksheet.write(map_row, 7, code, mapping_data_format)
                            worksheet.write(map_row, 8, name, mapping_data_format)
                            map_row += 1
...
with tab2:
    ...
    with col1:
        ...
        st.markdown("📝 **列印優化說明**：")
        st.success("已啟動 A4 極限微調排版，確保 40 人以上的大班級依然能完美塞進一頁 A4 之中！")
            ...
            with st.spinner("系統正在高速進行 5 倍收費放大與【A4自適應抗截斷排版】統計中..."):
                try:
                    ...
                    with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
                        ...
                        ws_details.set_column('A:A', 10) 
                        ws_details.set_column('B:B', 9)  
                        ws_details.set_column('C:C', 10) 
                        ws_details.set_column('D:D', 24) 
                        ws_details.set_column('E:E', 13) 
                        ws_details.set_column('F:F', 15) 
                        ws_details.set_column('G:G', 16) 
